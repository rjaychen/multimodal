...
if cap.isOpened(): print(cap.get(3), cap.get(4)) # print dimensions of camera
while cap.isOpened():
    ...
    if success:
        ...
        # print(results[0].boxes[0].xyxy.cpu().numpy())
        
        # annotated_frame = show_seg(results[0].masks.data)
        prompt_process = FastSAMPrompt(frame, results, device=DEVICE)
        
        # TODO: switch over to ultralytics FASTSam for tensor support; source version uses np arrays.

        # Everything
        ann = prompt_process.everything_prompt()

        # Bounding Box [x1, y1, x2, y2] -> can take one (bbox=) or multiple (bboxes=)
        # ann = prompt_process.box_prompt(bbox=[200, 200, 300, 300])

        # Text Prompt
        # ann = prompt_process.text_prompt(text='glasses')

        # Point Prompt points = [[x1,y1]] or [[x1,y1],[x2,y2]] ; pointlabel=[0] [1, 0] 0:background, 1:foreground
        # ann = prompt_process.point_prompt(points=[[300, 300]], pointlabel=[1])
